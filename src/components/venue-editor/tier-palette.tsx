...
export function TierPalette() {
  ...
  return (
    <div className="absolute left-4 top-4 z-10 flex flex-col gap-2 rounded-lg border border-border bg-card p-3 shadow-lg">
      <div className="mb-1 text-xs font-medium text-muted-foreground">Price Tiers</div>
      {TIERS.map((tier) => (
        <button
          key={tier.id}
          ...
          className={cn(
            ...
            "hover:bg-muted",
            selectedTier === tier.id && "bg-muted ring-2 ring-primary ring-offset-1 ring-offset-background",
          )}
        >
          <div
            className={cn(
      ...
      <div className="mt-2 border-t border-border pt-2">
        <p className="text-xs text-muted-foreground">Click on elements to assign the selected tier</p>
      </div>
    </div>
  )
}
