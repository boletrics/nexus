...
export function SeatRowElement({
	element,
	isSelected,
	onClick,
	onDragStart,
	...
	return (
		...
		>
			...
			{Array.from({ length: row.seats }).map((_, i) => {
				const angle = row.curved
					? (i - row.seats / 2) * (row.curveAngle / row.seats)
					: 0;
				...
				return (
					<g
						key={i}
						transform={`translate(${i * spacing}, ${yOffset}) rotate(${angle})`}
					>
						<rect
							width={seatWidth}
							height={seatWidth}
...
export function StandingAreaElement({
	element,
	isSelected,
	onClick,
	onDragStart,
