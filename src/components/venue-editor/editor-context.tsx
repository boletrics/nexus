...
import type { EditorMode, VenueElement, VenueMap, TierType, Position, ElementType } from "@/lib/types"
...
export function EditorProvider({ children }: { children: ReactNode }) {
  ...
  const updateElement = useCallback((id: string, updates: Partial<VenueElement>) => {
    setState((prev) => ({
      ...
      venue: {
        ...prev.venue,
        elements: prev.venue.elements.map((el) => (el.id === id ? ({ ...el, ...updates } as VenueElement) : el)),
      },
    }))
  }, [])
  ...
  const assignTierToElement = useCallback((id: string, tier: TierType) => {
    setState((prev) => ({
      ...
      venue: {
        ...prev.venue,
        elements: prev.venue.elements.map((el) => (el.id === id ? ({ ...el, tier } as VenueElement) : el)),
      },
    }))
  }, [])
